...
const int palette_colors = 1530; // 调色板颜色
...
public:
    ...
    int parentIndex;                  // 该格子的父节点在地图中的索引
    std::deque<int> neighbor_indices; // 该格子的邻居节点在地图中的索引

    int cellIndex; // 该格子在地图中的索引
};
...
/// @brief 提取地图中的墙的轮廓和障碍物的轮廓
/// @param original_map 输入的二值化地图图像或者灰度图
/// @param raw_wall_contours
/// @param raw_obstacle_contours
void ExtractRawContours(const cv::Mat &original_map, std::vector<std::vector<cv::Point>> &raw_wall_contours, std::vector<std::vector<cv::Point>> &raw_obstacle_contours)
{
    ...
    std::iota(wall_cnt_indices.begin(), wall_cnt_indices.end(), 0); // 从0开始，一直递增到 wall_cnt_indices.size()：
...
/// @brief 提取地图中的墙和障碍物轮廓，并对墙和障碍物进行处理，以适应给定的机器人半径。
/// @param original_map 输入的二值化地图图像
/// @param wall_contours
/// @param obstacle_contours
/// @param robot_radius 机器人的半径，如果不为零，则对墙和障碍物进行处理以适应机器人半径。
void ExtractContours(const cv::Mat &original_map, std::vector<std::vector<cv::Point>> &wall_contours, std::vector<std::vector<cv::Point>> &obstacle_contours, int robot_radius = 0)
{
    ...
    // 如果机器人半径不为零，则对墙和障碍物进行处理以适应机器人半径
    if (robot_radius != 0)
    {
        // 创建画布并绘制墙和障碍物轮廓
...
/// @brief 该函数用于将障碍物的轮廓点集转换为多边形列表
/// @param original_map 二值化的地图图像
/// @param obstacle_contours 障碍物的轮廓点集
/// @return
PolygonList ConstructObstacles(const cv::Mat &original_map, const std::vector<std::vector<cv::Point>> &obstacle_contours)
{
    PolygonList obstacles; // 多边形列表，存储转换后的障碍物多边形
    Polygon obstacle;      // 一个障碍物的多边形
    ...
    for (const auto &obstacle_contour : obstacle_contours)
    {
        ...
        // 将转换后的障碍物多边形添加到多边形列表中，并清空当前障碍物多边形，准备处理下一个障碍物
...
Polygon ConstructWall(const cv::Mat &original_map, std::vector<cv::Point> &wall_contour)
{
    ...
    else // 如果墙的轮廓点集为空，则生成默认的矩形墙
    {
        wall = ConstructDefaultWall(original_map);

        // 将默认墙的轮廓点集存储到wall_contour中
...
/// @brief  构建地图的cell_graph
/// @param original_map
/// @param wall_contours
/// @param obstacle_contours
/// @param wall 围墙的多边形
/// @param obstacles 障碍物的多边形列表
/// @return
std::vector<CellNode> ConstructCellGraph(const cv::Mat &original_map,
                                         ...
                                         const Polygon &wall, const PolygonList &obstacles)
{
    ...
    cv::fillPoly(map, wall_contours, cv::Scalar(255, 255, 255)); // 填充围墙的轮廓为白色
    cv::fillPoly(map, obstacle_contours, cv::Scalar(0, 0, 0));   // 填充障碍物的轮廓为黑色
...
/// @brief 该函数使用静态规划算法（Static Path Planning），为给定地图中的一个起始点规划一条路径，并返回路径信息。
/// @param map 输入的灰度图像，表示机器人运动的环境地图
/// @param cell_graph  由图像生成的图的节点（格子）信息。该向量的每个元素都包含该节点的中心点和周围节点的索引。
/// @param start_point 机器人的起始点
/// @param robot_radius 机器人的半径，用于考虑机器人能否通过某个区域
/// @param visualize_cells 如果设置为 true，则可视化绘制出每个节点（格子）
/// @param visualize_path 如果设置为 true，则可视化绘制出整个规划路径
/// @param color_repeats 用于可视化的颜色重复次数
/// @return  规划路径的序列，由每个格子的内部路径和链接路径构成。
std::deque<std::deque<Point2D>> StaticPathPlanning(const cv::Mat &map,
                                                   std::vector<CellNode> &cell_graph,
                                                   const Point2D &start_point,
                                                   int robot_radius,
                                                   bool visualize_cells,
                                                   bool visualize_path,
                                                   int color_repeats = 10)
{
    cv::Mat3b vis_map;
    ...
    int corner_indicator = TOPLEFT; // 角落指标

    int start_cell_index = DetermineCellIndex(cell_graph, start_point).front(); // 起点所在单元格的下标

    std::deque<Point2D> init_path = WalkInsideCell(cell_graph[start_cell_index], start_point, ComputeCellCornerPoints(cell_graph[start_cell_index])[TOPLEFT]); // 单元格内的初始路径
    local_path.assign(init_path.begin(), init_path.end());                                                                                                     // 将初始路径添加到局部路径中

    std::deque<CellNode> cell_path = GetVisittingPath(cell_graph, start_cell_index); // 计算所有访问的单元格的路径
    ...
    std::deque<Point2D> inner_path;            // 内部路径
    std::deque<std::deque<Point2D>> link_path; // 连接路径
    Point2D curr_exit;                         // 当前出口
    Point2D next_entrance;                     // 下一个入口

    std::deque<int> return_cell_path; // 返回的单元格路径
    std::deque<Point2D> return_path;  // 返回的路径
                       ...
                       std::deque<Point2D> &contouring_path, int robot_radius)
{
    ...
    while (!turning)
    {
        ...
        if (std::find(contouring_path.begin(), (contouring_path.end() - 1), next_pos) != (contouring_path.end() - 1) && contouring_path.size() > 1 && isObstacleCompleted)
        {
            return false;
        }
    }
...
Polygon GetNewObstacle(const cv::Mat &map, Point2D origin, int front_direction, std::deque<Point2D> &contouring_path, int robot_radius)
{
    ...
    while (keepContouring)
    {
        ...
        keepContouring = WalkAlongObstacle(map, obstacle_origin, origin, detecting_direction, direction_candidates, curr_pos, first_turning_direction, second_turning_direction, obstacle, new_obstacle, isObstacleCompleted, contouring_path, robot_radius);
...
void StaticPathPlanningExample3()
{
    ...
    map.setTo(255); // 将图像所有像素点赋值为255，表示全部为障碍物外的自由空间

    // 手工绘制多边形障碍物，将其填充为黑色区域
